...
import {SendPackedOperation, PackedUserOperation} from "../../script/SendPackedOperation.s.sol";
...
contract MinimalAccountTest is Test {
    ...
    function testEntryPointCanExecuteCommands() public {
        ...
        bytes memory functionalData = abi.encodeWithSelector(usdc.mint.selector, address(minimalAccount), AMOUNT);

        bytes memory executeCallData =
            abi.encodeWithSelector(minimalAccount.execute.selector, dest, value, functionalData);

        PackedUserOperation memory packedUserOp = sendPackedUserOpScript.generatedSingnatureOperation(
    ...
    function testValidationOfUserOps() public {
        ...
        uint256 validationData = minimalAccount.validateUserOp(packedUserOp, userOpHash, validateUserFunds);
        //Assert
    ...
    function testUserSignedOp() public {
        ...
        bytes memory executeCallData =
            abi.encodeWithSelector(minimalAccount.execute.selector, address(usdc), 0, functionDataForUSDCMint);

        PackedUserOperation memory packedUserOp = sendPackedUserOpScript.generatedSingnatureOperation(
            executeCallData, activeNetworkConfig, address(minimalAccount)
        );
    ...
    function testOwnerCanExecuteCommands() public {
        ...
        //Assert
        assertEq(usdc.balanceOf(address(minimalAccount)), AMOUNT, "MinimalAccount should have minited USDC");
    }
